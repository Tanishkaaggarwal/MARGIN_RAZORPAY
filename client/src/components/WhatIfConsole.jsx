...
import { Sparkles, Send, Calendar, AlertTriangle, ShieldCheck, CheckCircle2, XCircle, ArrowRight, RefreshCw, Zap } from 'lucide-react';
...
export function WhatIfConsole({ 
  ...
  const [mode, setMode] = useState('chat'); // 'chat' | 'direct'

  // Pre-configured quick demo queries that highlight different financial behaviors
  const quickChips = [
    { label: "Coffee & Lunch (₹450)", query: "Can I spend ₹450 on lunch today?", tag: "Safe" },
    { label: "Sneakers (₹4,500)", query: "Can I buy sneakers for ₹4,500?", tag: "Safe" },
    { label: "Sony XM5 Headphones (₹16,000)", query: "Can I spend ₹16,000 on noise cancelling headphones today?", tag: "Buffer Risk" },
    { label: "Goa Flight & Resort (₹28,000)", query: "Can I spend 28k on a Goa trip right now?", tag: "Deficit" },
    { label: "Gaming Laptop (₹65,000)", query: "Can I buy a laptop for ₹65,000 today?", tag: "Critical" }
  ];

  const handleSubmitChat = (e) => {
    e.preventDefault();
    if (!queryText.trim() || isLoading) return;
    onSimulate({ type: 'chat', text: queryText.trim() });
  };

  const handleQuickChipClick = (chipQuery) => {
    setQueryText(chipQuery);
    onSimulate({ type: 'chat', text: chipQuery });
  };
  ...
  const handleSubmitDirect = (e) => {
    ...
    onSimulate({
      ...
      description: directDescription || "Direct Spend Simulation"
    });
  };
  ...
  return (
    <div className="bg-fintech-card/95 border border-fintech-border rounded-2xl p-5 lg:p-6 shadow-xl relative overflow-hidden">
      ...
      <div className="flex items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Simulate any spending decision against upcoming bills, salary, and your ₹{user?.safetyBufferTarget?.toLocaleString('en-IN')} buffer.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center bg-slate-900 border border-slate-800 rounded-lg p-0.5 text-xs">
          ...
          <button
            ...
            className={`px-3 py-1 rounded-md transition-all font-medium ${
              mode === 'direct' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'
            }`}
          >
            Numeric Input
          </button>
      ...
      {/* Chat Query Form */}
      {mode === 'chat' ? (
        <form onSubmit={handleSubmitChat} className="relative mb-3">
          <input
            type="text"
            value={queryText}
        ...
        <form onSubmit={handleSubmitDirect} className="grid grid-cols-1 sm:grid-cols-12 gap-2 mb-3">
      ...
      {/* Quick Judge Demo Chips */}
      <div className="flex flex-wrap items-center gap-1.5 mb-4">
        <span className="text-[11px] text-slate-400 uppercase tracking-wider font-semibold mr-1">Demo Scenarios:</span>
      ...
      {simulationResult && (
        <div className={`mt-4 rounded-xl border p-4 sm:p-5 transition-all animate-in fade-in slide-in-from-top-2 duration-300 ${
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 border-b border-white/10 pb-3.5 mb-3.5">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <span className={`text-[11px] font-extrabold tracking-wider uppercase px-2 py-0.5 rounded font-mono ${
                  simulationResult.verdict === 'SAFE'
            ...
            {/* Simulated Purchase Value */}
            <div className="text-right md:min-w-[120px]">
              <span className="text-[11px] uppercase tracking-wider text-slate-400 block font-medium">Simulated Spend</span>
              <span className="text-lg font-bold font-mono text-white">
                ₹{simulationResult.purchase.amount?.toLocaleString('en-IN')}
              </span>
            </div>
          </div>
          ...
          <div className="text-sm text-slate-300 leading-relaxed mb-4">
            <p>{simulationResult.explanation.details}</p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-4 text-xs font-mono">
            <div className="bg-slate-900/80 border border-slate-800 rounded-lg p-2.5">
              <span className="text-[10px] text-slate-400 uppercase block font-sans">Baseline Min Balance</span>
              <span className="font-bold text-slate-200">
                ₹{simulationResult.metrics.baselineMinBalance?.toLocaleString('en-IN')}
              </span>
            </div>

            <div className="bg-slate-900/80 border border-slate-800 rounded-lg p-2.5">
              <span className="text-[10px] text-slate-400 uppercase block font-sans">Simulated Min Balance</span>
              <span className={`font-bold ${simulationResult.metrics.simulatedMinBalance < 0 ? 'text-rose-400' : 'text-slate-200'}`}>
                ₹{simulationResult.metrics.simulatedMinBalance?.toLocaleString('en-IN')}
              </span>
            </div>

            <div className="bg-slate-900/80 border border-slate-800 rounded-lg p-2.5">
              <span className="text-[10px] text-slate-400 uppercase block font-sans">Buffer Deficit</span>
              <span className={`font-bold ${simulationResult.metrics.simulatedBufferShortfall > 0 ? 'text-amber-400' : 'text-emerald-400'}`}>
                {simulationResult.metrics.simulatedBufferShortfall > 0 
                  ? `₹${simulationResult.metrics.simulatedBufferShortfall.toLocaleString('en-IN')}`
                  : '₹0 (Intact)'}
              </span>
            </div>

            <div className="bg-slate-900/80 border border-slate-800 rounded-lg p-2.5">
              <span className="text-[10px] text-slate-400 uppercase block font-sans">Health Score Impact</span>
              <span className="font-bold text-slate-300">
                {simulationResult.metrics.healthScoreBefore} → <span className={simulationResult.metrics.healthScoreAfter < 50 ? 'text-rose-400' : 'text-amber-300'}>{simulationResult.metrics.healthScoreAfter}</span>
              </span>
            </div>
          </div>

          {/* Earliest Safe Date Banner (If Breached) */}
          {simulationResult.earliestSafeDate && simulationResult.earliestSafeDate.date && (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-blue-300">Earliest Safe Date:</span>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-white/5">
            <p className="text-xs text-slate-400 flex items-center gap-1.5">
              ...
              <span>Simulated trajectory is currently overlaid on the 30-day chart below.</span>
            </p>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span>
                  {simulationResult.isSafe ? 'Razorpay Safe Checkout' : 'Verify Razorpay Shield'}
                </span>
